clean_mir_seq <- function(mat) {
  
  #z-score normalize the expression values
  mat <- scale(mat)
  #convert to long
  df <- as_tibble(mat, rownames = "mir") %>% 
    pivot_longer(-mir, names_to = "cell_line", values_to = "expression")
  return(df)
}

calc_target_matrix <- function(df){
  
  gene_list = as.character(unique(df$entrez_id)) #gotta coerce to a character
  load('./data_files/all_miRNA_targets.rda')
  
  overall_targeting_mat <- matrix(0, nrow=length(gene_list),ncol=length(names(targets_list)))
  
  rownames(overall_targeting_mat) <- gene_list
  ...
  ind = 1
  ind.max <- order(colSums(-overall_targeting_mat))[ind]
...
clean_target_matrix <- function(targeting_matrix = overall_targeting_mat, 
  ...
  targeting_df <- targeting_df %>% left_join(df) %>% 
  ...
  for(i in 1:length(cell_line_list)) {
    ...
    if(cancer_only == TRUE) {
      cell_line_df <- cell_line_df %>%
        ...
        slice_max(n = num_targets, order_by = mean_deltaGibbs)
    } else {
      cell_line_df <- cell_line_df %>%
